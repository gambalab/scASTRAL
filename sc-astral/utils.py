...
def filter_data(adata, min_cell_per_gene_pct=15, min_nonz_expr=1.12, min_detect_gene=.05, min_umi_per_cell=5000):
    """
    :param adata: Anndata Object containing raw seqdata
    ...
    cell_count_cutoff2 = np.log10(
        adata.shape[0] * min_detect_gene + 1e-7)  # genes detected in at least this will be included
    cells_per_gene = (adata.X > 0).sum(axis=0) + 1e-7  # computing count of cells per gene
    nonz_mean = adata.X.sum(axis=0) / cells_per_gene + 1e-7  # computing mean expression
    ...
    cell_filter = adata.X.sum(axis=1) > min_umi_per_cell  # computing the umi count per cell
